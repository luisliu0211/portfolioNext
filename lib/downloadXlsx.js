...
export const handleShifttoExcel = async (data) => {
  ...
  for (let i = 0; i < yarnNumber; i++) {
    ...
    sheet.getCell(`D${7 + i}`).value = data.yarnCost.yarnInfo[i].yarnPort / 100;
    ...
    // sheet.getCell(`I${7 + i}`).value = getSthById(data.yarnCost.yarnInfo[i].yarnUnit).title;
  }
  // // 紗規格
...
//下載 DB資料庫內容 在前端寫死
export const downloadDB = (title, data) => {
  // 创建一个工作簿
